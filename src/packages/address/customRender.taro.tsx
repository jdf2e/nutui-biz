...
export const CustomRender: FunctionComponent<Partial<AddressProps> & React.HTMLAttributes<HTMLDivElement>> = (
  props
) => {
      ...
      for (let index = 0; index < modelValue.length; index++) {
        if ((regionList as any)[tabName[index]].length === 0) {
          ...
          if (privateType === 'custom') {
            selectedRegionC[[tabName[index]] as any] = arr.filter((item: RegionData) => item.id === val)[0];
          } else if (privateType === 'elevator') {
            const sumArr: any = [];
            arr.forEach((item) => {
              sumArr.push(...(item as any).list);
            });
            selectedRegionC[[tabName[index]] as any] = sumArr.filter((item: RegionData) => item.id === val)[0];
          }
      ...
      <div className={b('region-tab')}>
        {selectedRegion &&
          Object.keys(selectedRegion).map((key: string | SelectedRegionType, index) => {
            return (
              <div
                className={`${b('tab-item')} ${index === tabIndex ? 'active' : ''} ${tabName[index]}`}
                key={index}
                ...
                return (
                  <li key={index} className={b('region-item')}>
                    <div
                      ...
                      {selectedRegion &&
                        selectedRegion[tabName[tabIndex] as SelectedRegionType].id === (item as RegionData).id && (
