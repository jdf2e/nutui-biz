...
import React, { useState } from "react";
import { Cell, ButtonProps } from "@nutui/nutui-react";
import { OrderCancelPanel, IKeyValue } from "@nutui/nutui-biz";
...
- 1、当数据 `cancelReason` 中 `key` 是 `other` 的时候，选择其他选项可以出现文本框。
- 2、设置 `canCancelReason` 为 `true`，可以取消已选择的原因。
...
const App = () => {
  ...
  const submitBtn = (selectedReason: IKeyValue, textAreaValue: string) => {
    console.log(
