...
config.nav.map((item) => {
  item.packages.forEach((element) => {
    ...
    if (taro || exportEmpty) {
    const filePath = path.join(`src/packages/${name.toLowerCase()}/index.taro.ts`);
    ...
    if (taro) {
    glob.sync(path.join(__dirname, `../../src/packages/${name.toLowerCase()}/`) + '*.md').map((f) => {
      let lang = 'zh-CN';
...
fs.outputFile(path.resolve(__dirname, '../../src/sites/doc/docs.taro.ts'), mdFileStr, 'utf8', (error) => {
  if (error) throw error;
});
